...
class UserQ18
  ...
  def introduce  
    ...
    @age>=20? "こんにちは、#{@name}と申します。よろしくお願いいたします。": "はいさいまいど〜、#{@name}です!!!"
...
class Zoo
  ...
  def info_entry_fee
    fee = case entry_fee
    when 0..3
      puts entry_fee[:infant]
    when 6..12
      puts entry_fee[:children]
    when 13..64
      puts entry_fee[:adult]
    when 65..120
      puts entry_fee[:senior]
    end
  end
  puts "#{user.age}さんの入園料は#{entry_fee}円です"
end
